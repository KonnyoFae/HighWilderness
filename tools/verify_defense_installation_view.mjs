...
const out=path.resolve(process.env.HW_DEFENSE_INSTALL_OUT??`artifacts/defense-installation-${Date.now()}`);await mkdir(out,{recursive:true});
...
try{
  ...
  await page.exposeFunction('__e3b_request',async req=>{
    if(req.method==='__choose_file')return request('editor.bind_file',{host_path:path.join(out,'defense-equipment.json'),mode:req.params.method},req.params.session_id,req.params.expected_revision);
  ...
  for(const [i,name] of ['近程自动拦截发射器','自持火控近程拦截发射器','红外自持火控30毫米近防炮'].entries()){
    ...
    await until(()=>snapshot.draft.modules.filter(m=>(m.prototype.id.includes('.interceptor')||m.prototype.id==='gtw.module.gun.5g.30mm.infrared')).length===i+1,'place '+name);
    assert(snapshot.preview.valid);
    const placed=snapshot.draft.modules.find(m=>(m.prototype.id.includes('.interceptor')||m.prototype.id==='gtw.module.gun.5g.30mm.infrared')&&m.placement.anchor_half_cell[0]===0&&m.placement.anchor_half_cell[1]===-8-i*2);
  ...
  await button('另存文件').click();await until(()=>snapshot.file_label==='defense-equipment.json'&&!snapshot.dirty,'save');
  const saved=structuredClone(snapshot.preview.model.layout);
  await button('返回编辑器入口').click();await button('打开舾装文件').click();await until(()=>snapshot.file_label==='defense-equipment.json','reopen');
  assert.deepEqual(snapshot.preview.model.layout,saved);assert(snapshot.preview.valid);assert.deepEqual(errors,[]);
  checks.push('All three defense placements retain exact geometry and prototype versions on save/reopen.');
  await writeFile(path.join(out,'result.json'),JSON.stringify({status:'DEFENSE_INSTALLATION_UI_PASS',checks},null,2));console.log(JSON.stringify({out,checks}));
