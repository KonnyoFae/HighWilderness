...
const out=path.resolve(process.env.HW_DISENGAGEMENT_OUT??`artifacts/tactical-disengagement-${Date.now()}`);await mkdir(out,{recursive:true});
...
try{
  ...
  await button('单舰脱队撤离').click();await until(()=>live.view.navigation.ships.find(s=>s.ship_id===escort)?.kind==='individual_withdrawal','detach');
  await button('取消单舰撤离').click();await until(()=>live.view.navigation.ships.find(s=>s.ship_id===escort)?.kind==='formation','cancel detach');
  ...
  assert.equal(live.view.navigation.disengagement.threshold_m,50000);
  await button('结束本场交战（测试）').click();await button('保存全部战后结果').click();
  await button('管理战后库存与下一场准备').click();
  await page.getByLabel('初始交战距离（公里）',{exact:true}).fill('50');await button('应用距离').click();
  await until(()=>packet.scene.distance_m===50000,'distance saved');
  await page.locator('.preparation-roster.player button').first().click();
  await page.getByText('编队位置与旗舰',{exact:true}).click();
  await page.getByLabel('舰艇航向（度）',{exact:true}).fill('180');await page.getByLabel('舰艇航向（度）',{exact:true}).press('Tab');
  await until(()=>Math.abs(packet.scene.sides[1].ships[0].heading_rad-Math.PI)<1e-8,'flagship faces away');
  await page.locator('.preparation-roster.enemy button').first().click();
  const properties=page.locator('.formation-properties');if(await properties.getAttribute('open')===null)await properties.locator('summary').click();
  await page.getByLabel('舰艇航向（度）',{exact:true}).fill('0');await page.getByLabel('舰艇航向（度）',{exact:true}).press('Tab');
  await until(()=>packet.scene.sides[0].ships[0].heading_rad===0,'enemy starts facing away');
  await button('配置双方舰内物资').click();await button('核对资源与预装填').click();await button('保存准备').click();
  await page.getByRole('heading',{name:'已保存准备结果',exact:true}).waitFor();await button('按当前编队进入交战').click();
  await button('开始交战').click();await until(()=>live?.status.fixed_step>1,'second scene starts');
  await page.locator(`[data-fleet-ship="${live.direct_ship_id}"]`).click();
  await button('整队撤离机动').click();await until(()=>live.view.gunnery.ending?.reason==='withdrawal','real distance exit');
  assert.equal(live.settlement.result.departures.length,2);
  assert.equal(live.settlement.result.escape_outcomes.length,0);
  await page.screenshot({path:path.join(out,'distance-settlement.png'),fullPage:true});
  await button('保存全部战后结果').click();await until(()=>live.settlement?.saved,'save distance result');
  checks.push('Single-ship detach/cancel; 50 km threshold; real propulsion crosses boundary; automatic settlement records both fleet departures; saved across consecutive encounters.');
  assert.deepEqual(errors,[]);
  await writeFile(path.join(out,'result.json'),JSON.stringify({status:'DISENGAGEMENT_UI_PASS',scope:'Real React/Python in Edge; injected file dialog, not native WebView',checks},null,2));console.log(JSON.stringify({out,checks}));
}catch(e){await writeFile(path.join(out,'last-state.json'),JSON.stringify(live));if(page){await page.screenshot({path:path.join(out,'failure.png'),fullPage:true});await writeFile(path.join(out,'failure.txt'),await page.locator('body').innerText());}throw e;}
