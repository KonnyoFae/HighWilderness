// Stage 1: independent tactical entry, production React/sidecar, isolated persisted ships.
...
const out=path.resolve(process.env.HW_TACTICAL_STAGE1_OUT??'artifacts/tactical-stage1-20260915');
let backend,browser,page,serial=0,scene,packet;
const calls=[];
let dropEntryModeReply = true, dropReturnModeReply = true;
let dropRecoverySaveReply = false;
let dropDeployReply = true, dropEntryLookupReply = true;
...
const url=process.env.HW_TACTICAL_STAGE1_URL??'http://127.0.0.1:1423/e3b-test.html?entry=tactical';
start();
try{
  ...
  await page.exposeFunction('__e3b_request',async req=>{
    ...
    if(req.method==='tactical.set_mode'){
      if(req.params.mode==='tactical'&&dropEntryModeReply){dropEntryModeReply=false;throw new Error('测试：入战切换回执丢失');}
      if(req.params.mode==='editor'&&dropReturnModeReply){dropReturnModeReply=false;throw new Error('测试：返回准备回执丢失');}
    }
    if(req.method==='tactical.realtime.save'&&dropRecoverySaveReply){dropRecoverySaveReply=false;throw new Error('测试：战果保存回执丢失');}
    if(req.method==='tactical.realtime.deploy_prepared'&&dropDeployReply){dropDeployReply=false;throw new Error('测试：部署回执丢失');}
    if(req.method==='tactical.realtime.prepared_entry'&&dropEntryLookupReply){dropEntryLookupReply=false;throw new Error('测试：入战查询回执丢失');}
  ...
  assert.deepEqual(prepared.ships[0].lift_reserve,packet.ships[0].lift_reserve);
  assert((await page.getByRole('region',{name:'升力冗余',exact:true}).innerText()).includes('干重'));
  ...
  await button('重试场景切换').waitFor();await button('重试场景切换').click();
  await page.getByRole('alert').filter({hasText:'入战结果尚未确认'}).waitFor();
  assert(await button('返回战前准备').isDisabled());
  await button('重试进入战场').click();
  ...
  assert.deepEqual(scene.view.ships[0].lift_reserve,prepared.ships[0].lift_reserve);
  const flagship=scene.direct_ship_id;
  await button('红方测试舰').click();
  assert((await page.getByRole('complementary',{name:'所选舰艇'}).innerText()).includes('红方测试舰'));
  assert.equal(scene.direct_ship_id,flagship);
  await button(prepared.ships[0].name+' · 旗舰').click();
  await button('开始交战').click();await button('火炮').click();await page.getByLabel('所控火炮',{exact:true}).selectOption(gun().module_id);
  ...
  await button('舰况').click();
  await page.screenshot({path:path.join(out,'battle.png'),fullPage:true});
  assert(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth));
  await button('火炮').click();
  assert((await page.getByLabel('所控火炮',{exact:true}).boundingBox()).width>200);
  await page.screenshot({path:path.join(out,'battle-weapons.png'),fullPage:true});
  checks.push('Shared battlefield selection leaves direct flagship unchanged; dry-mass lift readings agree across preparation and combat; compact layout has no horizontal overflow');
  ...
  await button('结束本场交战').click();
  const nextPreparation=button('管理战后库存与下一场准备');await nextPreparation.waitFor();
  assert(await nextPreparation.isDisabled());
  assert.equal(await page.getByText('技术测试舰 · 再次交战',{exact:true}).count(),0);
  ...
  assert(await nextPreparation.isEnabled());await nextPreparation.click();
  await button('重试场景切换').waitFor();await button('重试场景切换').click();
  await button('准备所选舰船').waitFor();
  const released=await request('tactical.preparation.library',{});assert(released.ships.every(s=>!s.blocked));
  assert(!calls.includes('tactical.realtime.deploy'));
  await page.screenshot({path:path.join(out,'returned-preparation.png'),fullPage:true});
  checks.push('Prepared settlement hides legacy ship deployment despite existing legacy saves; save gates return; returning releases battle claims and refreshes preparation');
  ...
  await button('核对资源与预装填').click();await button('保存准备').click();
  await page.getByRole('heading',{name:'已保存准备结果',exact:true}).waitFor();
  await button('以所选舰为旗舰进入交战').click();await page.getByText('战场记录与全舰资源',{exact:true}).click();await firePanel.waitFor();
  ...
  const pendingResult=structuredClone(scene.settlement.result);
  await page.goto('about:blank');await stop();start();await hello();
  dropRecoverySaveReply=true;
  await page.goto(url);await page.getByRole('region',{name:'恢复战后结果',exact:true}).waitFor();
  await button('保存全部战后结果').click();
  await page.getByRole('alert').filter({hasText:'战果保存回执丢失'}).waitFor();
  await button('保存全部战后结果').click();
  await page.getByRole('status').filter({hasText:'已保存全部参战舰船'}).waitFor();
  assert.deepEqual((await request('tactical.realtime.settlement',{settlement_id:pendingResult.settlement_id})).result,pendingResult);
  await page.screenshot({path:path.join(out,'recovered-settlement.png'),fullPage:true});
  await button('管理战后库存与下一场准备').click();await button('准备所选舰船').waitFor();
  assert((await request('tactical.preparation.library',{})).ships.every(s=>!s.blocked));
  checks.push('Independent preparation recovers pending results after backend exit; a lost save reply retries without duplicate changes and releases ships');
  assert.equal(await button('战术视角').count(),0);
  assert(!calls.includes('tactical.create'));assert(!calls.includes('tactical.realtime.create'));
  checks.push('Independent entry opens preparation directly; lost entry/return mode replies recover; legacy trial routes are absent');
  assert(!calls.includes('tactical.realtime.deploy'));assert.deepEqual(errors,[]);
  await writeFile(path.join(out,'result.json'),JSON.stringify({status:'TACTICAL_STAGE1_UI_PASS',checks,result,calls},null,2));
  console.log(JSON.stringify({status:'TACTICAL_STAGE1_UI_PASS',checks}));
}catch(error){
