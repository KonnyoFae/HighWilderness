// 5h: place all eight matching turret launchers from the real editor catalog; save/reopen.
...
const out=path.resolve(process.env.HW_CATALOG_INSTALL_OUT??`artifacts/missile-catalog-installation-${Date.now()}`);await mkdir(out,{recursive:true});
...
try{
  ...
  await page.exposeFunction('__e3b_request',async req=>{
    if(req.method==='__choose_file')return request('editor.bind_file',{host_path:path.join(out,'missile-launchers.json'),mode:req.params.method},req.params.session_id,req.params.expected_revision);
  ...
  for(const [i,name] of ["火箭雷达小型导弹发射器", "火箭反辐射小型导弹发射器", "火箭红外小型导弹发射器", "火箭复合小型导弹发射器", "涡喷雷达小型导弹发射器", "涡喷反辐射小型导弹发射器", "涡喷红外小型导弹发射器", "涡喷复合小型导弹发射器"].entries()){
    ...
    const p=await page.evaluate(async({regions,width,height,y})=>{const {fit,screen}=await import('/src/editor/viewport.ts');return screen({x:0,y},fit(regions,width,height));},{regions,width:vb[2],height:vb[3],y:-20-i*5});
    await page.mouse.click(box.x+p.x*box.width/vb[2],box.y+p.y*box.height/vb[3]);
    await until(()=>snapshot.draft.modules.filter(m=>m.prototype.id.startsWith('gtw.module.launcher.5c.small.')).length===i+1,'place '+name);
    assert(snapshot.preview.valid);
    const placed=snapshot.draft.modules.find(m=>m.prototype.id.startsWith('gtw.module.launcher.5c.small.')&&m.placement.anchor_half_cell[0]===0&&m.placement.anchor_half_cell[1]===-8-i*2);
    const m=snapshot.preview.model.layout.modules.find(m=>m.id===placed?.id);
    assert(m);assert(m.internal_cells.length>0);assert.equal(m.top_cells.length,1);assert.equal(m.clearance_spatial_keys.length,0);
    checks.push(name+' placed through real catalog and canvas, with the correct internal and exposed footprints.');
  }
  await button('↖ 拖动部件').click();await page.mouse.move(20,20);await page.screenshot({path:path.join(out,'installed.png'),fullPage:true});
  await button('另存文件').click();await until(()=>snapshot.file_label==='missile-launchers.json'&&!snapshot.dirty,'save');
  const saved=structuredClone(snapshot.preview.model.layout);
  await button('返回编辑器入口').click();await button('打开舾装文件').click();await until(()=>snapshot.file_label==='missile-launchers.json','reopen');
  assert.deepEqual(snapshot.preview.model.layout,saved);assert(snapshot.preview.valid);assert.deepEqual(errors,[]);
  checks.push('All eight missile launcher placements retain exact geometry and prototype versions on save/reopen.');
  await writeFile(path.join(out,'result.json'),JSON.stringify({status:'MISSILE_CATALOG_INSTALLATION_UI_PASS',checks},null,2));console.log(JSON.stringify({out,checks}));
