...
const out = path.resolve(process.env.HW_GUN_OUT ?? "artifacts/p3-settlement-browser");
await mkdir(out, { recursive: false });
let backend;
const pending = new Map(), errors = [], checks = [];
let serial = 0, scene = null, initial = null, lostGunReply = false, lostSaveReply = false;
function startBackend() {
  backend = spawn(process.env.HW_PYTHON ?? "python", ["-X", "utf8", "-m", "backend.high_wilderness_sidecar", "--instance-id", "backend.e3bbrowser", "--settlement-dir", path.join(out, ".local", "store")], { cwd: process.cwd(), windowsHide: true });
  ...
  createInterface({ input: backend.stdout }).on("line", line => {
    ...
    if (value.ok === false) errors.push(JSON.stringify(value.error));
...
let browser, page;
try {
  await request("system.hello", { client_name: "p3.browser", client_version: "1", supported_interfaces: ["gaotian.web-bridge/v1alpha1"], required_capabilities: ["tactical.realtime.gun"] });
  ...
  await page.exposeFunction("__e3b_request", async req => {
    ...
    if (lostSaveReply && req.method === "tactical.realtime.save") { lostSaveReply = false; throw new Error("fixture: save committed but reply lost"); }
  ...
  assert.equal(scene.view.gunnery.damage_enabled, true);
  assert(scene.view.gunnery.damage.hits > 0);
  assert(scene.view.ships.some(s => s.hull_integrity < 1));
  assert(scene.view.gunnery.weapons.some(g => g.ship_id === "ship.web.red" && g.shots > 0));
  await button("结束交战 / 撤离").click();
  await until(() => scene.view.gunnery.ending?.reason === "withdrawal", "withdrawal freezes battle");
  assert.equal(scene.view.gunnery.projectiles.length, 0);
  assert.equal(scene.status.pause_reason, "battle_finished");
  assert.equal(await page.getByRole("alert").count(), 0);
  assert(await button("开始试航").isDisabled());
  await page.screenshot({ path: path.join(out, "ended.png"), fullPage: true });
  assert.deepEqual(errors, []);
  checks.push("pause freezes simulation; real shell hits reduce hull/module HP; enemy returns fire; withdrawal removes projectiles and disables resume");
  const first = structuredClone(scene.settlement.result);
  assert.equal(scene.settlement.saved, false);
  assert.equal(await button("退出实时实验").isDisabled(), true);
  await page.screenshot({ path: path.join(out, "pending-settlement.png"), fullPage: true });
  await page.goto("about:blank");
  await page.waitForTimeout(100);
  const stoppedBackend = backend;
  await new Promise(resolve => { stoppedBackend.once("exit", resolve); stoppedBackend.kill(); });
  scene = null; initial = null;
  startBackend();
  await request("system.hello", { client_name: "p3.restart", client_version: "1", supported_interfaces: ["gaotian.web-bridge/v1alpha1"], required_capabilities: ["tactical.realtime.save"] });
  await page.goto(process.env.HW_GUN_URL ?? "http://127.0.0.1:1421/e3b-test.html");
  await button("战术视角").click();
  await button("实时试航（实验）").click();
  ...
  await page.getByRole("alert").filter({ hasText: "save committed but reply lost" }).waitFor();
  const committed = await request("tactical.realtime.settlement", { settlement_id: first.settlement_id });
  assert.equal(committed.saved, true);
  await button("保存全部战后结果").click();
  await button("结算已保存").waitFor();
  const afterRetry = await request("tactical.realtime.settlement", { settlement_id: first.settlement_id });
  assert.deepEqual(afterRetry, committed);
  await page.screenshot({ path: path.join(out, "saved-settlement.png"), fullPage: true });
  checks.push("pending result survives actual backend restart; lost save reply can be retried without repeated revision or consumption");
  const savedBlue = first.ships.find(s => s.after.ship_id === "ship.web.blue").after.state;
  await button("使用舰船 1 进入新交战").click();
  await until(() => scene && scene.status.epoch !== first.scene_id, "new scene from saved ship");
  const blue = scene.view.ships.find(s => s.id === "ship.web.blue");
  assert.equal(blue.hull_integrity, savedBlue.hull_integrity_fraction);
  assert.deepEqual(blue.modules.map(m => [m.id,m.durability]), savedBlue.modules.map(m => [m.module_id,m.durability_points]));
  assert.equal(gun().ammo_resources, savedBlue.magazines.reduce((n,m) => n+m.quantity, 0));
  assert.equal(gun().ready_rounds, savedBlue.weapons[0].ready_rounds);
  assert.equal(gun().target_ship_id, null); assert.equal(gun().shots, 0);
  assert.deepEqual(blue.velocity_mps, [0, 0]);
  assert.equal(scene.view.gunnery.projectiles.length, 0);
  await page.screenshot({ path: path.join(out, "next-battle.png"), fullPage: true });
  await button("开始试航").click();
  await page.waitForTimeout(250);
  await button("结束交战 / 撤离").click();
  await button("保存全部战后结果").click();
  await until(() => scene.settlement?.saved, "second battle saved");
  const second = scene.settlement.result;
  assert.equal(second.ships[0].after.state.instance_id, savedBlue.instance_id);
  assert.equal(second.ships[0].after.state.revision, 2);
  assert.equal(second.ships[0].after.state.hull_integrity_fraction, savedBlue.hull_integrity_fraction);
  checks.push("saved damaged hull, modules and ammunition enter and run a new battle; aim/velocity reset, same ship saves revision 2");
  assert.deepEqual(errors, []);
  await writeFile(path.join(out, "battle-a.json"), JSON.stringify(first, null, 2));
  await writeFile(path.join(out, "battle-b.json"), JSON.stringify(second, null, 2));
  await writeFile(path.join(out, "result.json"), JSON.stringify({ status: "PASS", scope: "Real Python stdio + React/Pixi in headless Edge, real hits and ending, no native WebView claim", checks, final: scene, errors }, null, 2));
