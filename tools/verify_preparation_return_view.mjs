...
const out=path.resolve(process.env.HW_OP1_OUT??'artifacts/op1-preparation-return-v1');
...
const url=process.env.HW_OP1_URL??'http://127.0.0.1:1423/e3b-test.html';
start();
try{
  ...
  await page.getByText('技术测试舰 · 再次交战',{exact:true}).waitFor();
  assert(await button('使用舰船 1 进入新交战').isEnabled());
  checks.push('After second prepared battle, legacy testing route remains separate and available');
  assert(!calls.includes('tactical.realtime.deploy'));assert.deepEqual(errors,[]);
  await writeFile(path.join(out,'result.json'),JSON.stringify({status:'OP1_PREPARATION_RETURN_UI_PASS',checks,result,calls},null,2));
  console.log(JSON.stringify({status:'OP1_PREPARATION_RETURN_UI_PASS',checks}));
}catch(error){
