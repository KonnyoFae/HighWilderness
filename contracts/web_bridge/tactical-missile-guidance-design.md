...
实施顺序、跨文档衔接及剩余事项统一见[工作包 5a～5i](./tactical-stage5-implementation-plan.md)，当前实现边界见[5h 记录](./tactical-stage5h-catalog-joint-acceptance.md)。下一工作包为 6 舰队指挥、撤离和战果接口。

2026-09-17 已明确：当前导弹低于型号参考速度后逐渐失去转向能力，零速不能原地转向；有动力时可沿现朝向重新加速，寿命一直推进。未来矢量推进型号使用独立控制律。首次搜索只捕获作用层目标；既有锁定目标换层后，按其新层天气判断续锁，弹体本身及命中作用层不变。
