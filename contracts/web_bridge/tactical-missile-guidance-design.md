...
日期：2026-09-17。状态：5a～5g 已接通；四类常规制导、记忆搜索、数据链及电子对抗见[5f 记录](./tactical-stage5f-guidance-ew.md)，专用拦截弹、进阶防御设备及本轮确认的分配/补射规则见[5g 记录](./tactical-stage5g-interceptor-defense.md)。5h 全目录与四舰联合验收已通过，见[5h 记录](./tactical-stage5h-catalog-joint-acceptance.md)。5i 连续两场及恢复已接通，见[5i 记录及后续交接](./tactical-stage5i-continuity-handoff.md)。半主动／旧光电实战型号已确认列为后续扩展；下文保留历史讨论。

实施顺序、跨文档衔接及剩余事项统一见[工作包 5a～5i](./tactical-stage5-implementation-plan.md)，当前实现边界见[5h 记录](./tactical-stage5h-catalog-joint-acceptance.md)。[5j 导弹换层追击](./tactical-missile-layer-pursuit-implementation-plan.md)现已完成范围内验收，接续工作包 6 舰队指挥、撤离和战果接口。

2026-09-17 已明确：当前导弹低于型号参考速度后逐渐失去转向能力，零速不能原地转向；有动力时可沿现朝向重新加速，寿命一直推进。未来矢量推进型号使用独立控制律。首次搜索只捕获作用层目标；既有锁定目标换层后，按其新层天气判断续锁，旧实现的弹体固定层限制已由 5j 在途换层实现覆盖。
...
2026-09-17 修订：有效锁定或支持的数据链情报可支持实际换层追击，完成前属于原层、完成后切到目标层，不能命中异层目标。保留跨层发射降速；无动力寿命改为型号固定时长，不再按发射层或途经层选取或重算；发射舰或观察画布层也不决定锁定天气。失锁后的记忆仍不能读取未知目标的新位置或机动。
...
直飞失的弹沿用预定计时自毁规则，动力耗尽不立即坠毁；在途换层及低速回落遵守新专题，不以失锁作为读取隐藏目标的理由。8 字搜索必须实际飞行和转向，受最大过载、低速能力及每 1G·秒损失 9.8 米/秒的既定规则约束；主发动机仍工作时可补偿，无动力时持续耗速。

制导状态与助推/主发动机/无动力状态分别运行：导引头重捕、二次搜索或数据链更新不能重启发动机、补回速度、重置寿命或瞬时改变所属层；助推段即使已获得目标信息，也不能执行转向机动。目标换层后可按新层天气维持感知，弹体可按新专题实际换层追击，首次搜索的现有层过滤与新追击规则需在实施时衔接。
...
已确认：支持数据链的导弹允许装有相应设备的任意友舰接力提供有效目标情报，包括重新指定目标；数据链更新可支持实际换层追击，但不能直接瞬时改层；每时刻只在当前唯一所属层命中或被拦截。舰队核心对舰船指令的抗干扰设定不自动扩展到导弹数据链。
...
数据链与指挥机的挂载、备份安装和战损关系需要接入模块系统。用户已确认：即使本舰雷达和红外设备全部不可用，只要指挥机与数据链正常，友舰共享的有效跟踪数据也可替代本舰传感器来源，支持普通火控与拦截；不再要求接收舰必须同时有一部本地传感器。观测来源丢失或数据过期后不能继续当作实时精确跟踪。传输范围/时延、情报有效性、干扰与接力时序、给在途导弹的数据更新和自主重捕关系继续细化。改攻其他目标的能力已确认，不再作为行为待定项；数据链不能替代半主动武器要求的实际照射。任何更新都继续遵守实际换层边界、剩余动力/固定寿命及相应导引头工作条件。
...
原目标被其他武器消灭后，在途拦截弹可继续搜索并追踪新的合法威胁。新目标须由该弹有效导引头观测或其支持的合法数据链情报取得，继续受目标资格、制导/照射条件及实际换层边界约束；不能读取全场隐藏弹体。换靶不重新发射、不退弹，也不恢复动力、速度或寿命，真实转弯照常耗速；原目标已消灭时不再坚持原目标重捕优先。没有可捕获新目标时按型号已有失锁/搜索规则继续，寿命到期自毁。
...
### 5c 后勤规则确认与接入（2026-09-17）

战前核对显示所需总时间，保存时统一完成准备；每舰按计划顺序操作，各舰同步，库内并行组装仍按型号/设备配置。取消进行中的组装或原料装填全额返还原配方并清零进度；整装装填取消则退回同一枚弹，满库先腾位、期间保留退回作业。返还原料允许暂时超过货舱容量，不能继续新增货物装载。

每库仍只选一种型号，同型号不同战斗部独立计数、共用容量；有实际入库需求时按弹体编号从早到晚拆解现有库存，不因库存满而自行持续拆弹。已启动的原料装填保持原料来源，不因途中出现整装弹自动改道；下一次装填重新按整装优先选择。无完好同型号库时整装弹不能凭空退库，保持退回作业等待；主动取消卸弹则恢复到发射器。

5c 已接入这些规则以及导弹库毁伤、未完成作业战后保存。具体当前容量、时长、成本和在制殉爆折算见[5c 实施记录](./tactical-stage5c-missile-logistics.md)；本节之前“继续细化”的相关历史条目以此更新为准。实际发射、飞行、制导与观测仍未接入，专用拦截弹动力/导引头仍待 5g 决定。
...
13. 制导新增验收：多候选时最近大目标优先、重捕优先原目标但不绕过有效性；目标换层后按新层天气保留/失去锁定，导弹完成实际相邻层机动后才改变飞行/命中层，型号固定无动力寿命不刷新；低级被动/半主动型号直飞后仍能在视锥出现信号时重捕，高级型号只飞向最后有效区域而不读取隐蔽机动。
...
雷达/IRST、指挥机、数据链备份、独立跟踪额度与真实火控/设备页已接通，见[实施记录](./tactical-stage5d-observation.md)。本轮明确：红外同样按目标特征占用额度；满额优先保留已有跟踪，战损减容后先保留玩家锁定与来袭威胁；全部来源失效立即停止实时火控/自动拦截，短暂保留最后观测，其他有效来源可接替。情报共享不代替实际照射或弹载锁定。上文涉及对应已定事项的“继续细化”由此记录收敛，导弹飞行、EW 区域与制导机仍按 5e/5f 推进。


## 2026-09-17：5g 确认与接入

专用弹为火箭复合导引，进阶发射器内置雷达，进阶近防炮内置红外，各带本武器专用计算。内圈按受威胁舰当前可工作的近防炮最远有效射程，无可用炮则不设内圈；目标进内圈后停止自动新发，已发射弹继续飞，近防补足伤害。本舰共享在途分配，有效数据链友舰之间共享，手动单发也记入；默认一枚，只有已命中未摧毁、落空或失效后才允许符合条件的自动补射。无数据链的不同舰艇可能重复分配。

上述规则均经用户确认并完成首版接入，覆盖原文尚待细化的对应条目。参数、开关、兼容与验证见[5g 实施记录](./tactical-stage5g-interceptor-defense.md)，后续进行 5h 全目录联合验收。
