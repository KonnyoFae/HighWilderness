...
> 2026-09-12 O4 状态更正：用户已确认 O4 测试实际完成，旧待验收/待留档记录有误，当前为 `O4_COMPLETED_USER_CONFIRMED`，不再安排 O4 收尾。见[验收状态更正](./contracts/web_bridge/o4-outfit-ui-acceptance.md)。下文 O4 未完成表述仅保留为历史记录，不代表当前待办；原自动测试与文件检查结果不变。

> 2026-09-07 接续状态：W2a/W2b/W3a、H1—H4 最小范围已完成，三条船壳真实 UI 往返与镜像/装甲、多甲板验收通过；H5a、O1/O2、O3a—O3c 舾装画布与武器组最小范围已完成，舾装用户功能复测认可，O4 规范实机留档收尾；T1a 两舰场景与表现接口已接通，下一步 T2a/T3a 战术画布和直控；H5b 净容量关系待冻结，填充运行效果分片后续接入。以下重排说明中的 W2a 起点为调整当时记录。实际验收及历史指纹回归限制详见 Web 主计划完成记录。

## 现行状态与调度补充（2026-09-07）

W0/W1 已完成，d4 完整安全链已验收；以下 W1 尚未实现的描述是 W0 建立时的记录。当前按 Web 实施计划第零章从 W2a 开始，优先船壳/舾装编辑器和两舰基本战术。正式 T0 性能矩阵不再是编辑器或 T1a 的前置。

T1a 先定义独立版本的表现全量快照/事件/输入，差量后移；T3a 保留 60 Hz 权威固定步、串行领域所有权与有界 I/O，首版目标为 10—20 Hz 表现快照。W0 的受控路径、严格资源验证、修订冲突、超时未知结果和可靠事件边界不变。该调度补充优先于下文旧实施顺序，不表示既有协议样例自动成为新业务实现。
