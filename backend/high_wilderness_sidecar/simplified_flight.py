...
REASONS = ("actuator_destroyed", "host_destroyed", "power_unavailable", "crew_unavailable",
           "fuel_unavailable", "engine_tripped", "mode_disabled", "emergency_cut", "lifecycle_unavailable")
...
class PropulsionKernel:
    ...
    def boundary(self, source, n, requested, events, *, load=None, profile=None, overg=False, crew_lock=True):
        ...
        for i in sorted(updates):
            ...
            if any(old.blocked) != any(new.blocked):
                ...
                new = replace(new, engine=EngineRuntimeState(new.engine.actuator_instance_id,
                    new.engine.actuator_category, "off", "stop" if new.engine.actuator_category == "main_engine" else None,
                    0, 0, None, None))
        ...
        for i in sorted(touched):
            ...
            if target != state.target_output_percent or state.next_transition_step == n or state.commanded_notch != notch:
...
class SimplifiedFlightSession:
    def __init__(self, seeds, safety_profile, *, direct_ship_id):
        seeds = tuple(seeds)
        ...
        ships = []
        for seed, kernel in zip(seeds, self._kernels):
            ...
            ships.append(FlightShip(seed.contributions.ship_id, seed.motion,
                kernel.initial(seed.initially_ready, seed.initial_blockers), directional_control(),
                seed.contributions.ship_id == direct_ship_id))
    ...
    def step(self, control=None, *, ship_id=None, events=(), authority_events=(), project=None):
        ...
        try:
            ...
            for seed, kernel, original in zip(self._seeds, self._kernels, before.ships):
                ship = original
                ...
                for phase in ("opening", "closing"):
                    ...
                    drag = dynamics.calculate_tactical_drag(seed.model, ship.motion)
                    def load(outputs):
                        metrics = dynamics._load_metrics(seed.model, ship.motion, actuation(seed.contributions, outputs),
                    ...
                    state, facts = kernel.boundary(ship.propulsion, n, requested, device_events,
                        ...
                        overg=selected.overg_requested, crew_lock=seed.model.runtime.crew_safety_lock_enabled)
                    ...
                    if phase == "opening":
                        ...
                        motion, diagnostic = dynamics._integrate_delivered_actuation(seed.model, ship.motion,
...
def build_sample_session(root):
    ...
    for ship, resources, table in zip(old.world.scene.ships, old.resources.ships, old.resources.simplified_propulsion):
        ...
        seeds.append(ShipSeed(table, model, ship.motion_state))
