...
export function EditorPanel({ transport, instance }: { transport: BridgeTransport; instance: string }) {
  ...
  async function run(method: string, params: Record<string, unknown> = {}) {
    ...
    setNotice("");
    try {
      if (method === "__open_file" || method === "__save_as" || method === "__new_version") {
        const opening = method === "__open_file";
        ...
        params = opening ? { destination_handle: grant.destination_handle }
          : { destination_handle: grant.destination_handle, new_version: method === "__new_version" };
        method = opening ? "editor.open_file" : "editor.save";
      }
  ...
  return <section className="panel editor-panel" aria-label="船壳编辑会话">
    <div className="editor-heading"><div><p className="panel-kicker">船坞 · 编辑基础</p>
      <h2>船壳编辑会话</h2></div><span className="muted">技术测试资源</span></div>
    <p className="editor-note">合法船壳可保存为 JSON 文件。已应用的修改自动记录为恢复草稿；测试资源请另存。</p>
    {!session && recoveries.length > 0 && <div className="editor-summary"><strong>可恢复的草稿</strong>
      {recoveries.map(record => <button key={record.key} disabled={busy || !record.valid_record}
        onClick={() => void run("editor.recover", { recovery_key: record.key })}>恢复：{record.name}</button>)}</div>}
    <div className="editor-row">
      <label>船壳资源<select aria-label="船壳资源" value={selected} disabled={busy || session !== null}
      ...
      <button disabled={!selected || busy || session !== null}
        onClick={() => void run("editor.open", { resource_key: selected })}>打开船壳</button>
    ...
    {session && <>
      <div className="editor-row">
        ...
        <button disabled={busy || !name.trim() || name === session.draft.name}
          onClick={() => void run("editor.command", { command: "hull.rename", arguments: { name } })}>应用名称</button>
      </div>
      ...
      <div className="editor-summary" aria-live="polite">
        <strong>{localDraftBusy ? "本地编辑中，待提交检查" : session.preview.valid ? "船壳合法" : "草稿有待修正"}</strong>
        <span>修订 {session.revision} · {session.dirty ? "未保存修改" : "与源资源一致"}</span>
        <span>{session.draft.decks.length} 层甲板</span>
      ...
      <HullViewport key={session.session_id} session={session} busy={model.pending !== null}
