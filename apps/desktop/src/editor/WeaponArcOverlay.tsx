...
export function arcVisibleAtLevel(arc: WeaponArc | undefined, module: LayoutModule | undefined, level: number | undefined) {
  return level !== undefined && !!arc?.origin_m && arc.status !== "placement_invalid"
    && arc.status !== "vertical_launch" && arc.status !== "countermeasure_deployment" && arc.status !== "launch_policy_unavailable"
    && (arc.base_deck_level === level || !!module && visibleAtLevel(module, level));
}

export function WeaponArcOverlay({ arc, module, level, camera, show, sensor=false }: {
  arc?: WeaponArc; module?: LayoutModule; level?: number; camera: Camera; show: boolean; sensor?: boolean;
}) {
  ...
  if (arc.status === "hull_occlusion_resolved") return <g pointerEvents="none" aria-label={sensor?'传感器水平视界':'武器水平射界'}>{[
    { intervals: arc.intervals_deg ?? [], color: "#70dfa1", label: sensor?'水平可探测方向；与盲区共用的边界不可探测':"水平可射方向；与禁射区共用的边界禁止开火" },
    { intervals: arc.blocked_intervals_deg ?? [], color: "#ff6868", label: sensor?'上层船壳遮挡，无法探测（含边界）':"上层船壳遮挡，禁止开火（含边界）" },
