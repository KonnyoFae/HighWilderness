...
export interface NavigationView {
  ...
  disengagement?:{defending_side_id:string;effective_layer:string;threshold_m:number;distance_m:number;individual_distances_m?:Record<string,number>;waiting_ship_ids:string[];departed_ship_ids:string[]}|null;
}
export type NavigationIntent={kind:'move'|'hold'|'return'|'attack'|'withdraw'|'cancel_withdraw'|'detach'|'cancel_detach';arguments:Record<string,unknown>};
export function FleetOrders({view,shipId,directId,disabled,picking,onPick,onCommand,speed,onSpeed,heading,onHeading}:{
  ...
  const detached=order?.kind==='individual_withdrawal',distanceState=n.disengagement;
  const departed=distanceState?.departed_ship_ids.includes(shipId);
  disabled=disabled||!!departed;
  ...
  return <section className="fleet-orders" aria-label="舰队命令">
    <h4>舰队命令</h4><p role="status">{departed?'已离场':detached?'正在单独撤离':retreat?`整队撤离机动 · 共同目标速度 ${retreat.speed_mps.toFixed(1)} m/s`:shipId===directId?'旗舰使用独立操纵区':labels[order?.status??'following']}</p>
    {distanceState&&<p>旗舰距离 {(distanceState.distance_m/1000).toFixed(2)} 公里 · 脱离阈值 {(distanceState.threshold_m/1000).toFixed(0)} 公里{distanceState.waiting_ship_ids.length?` · 等待 ${distanceState.waiting_ship_ids.length} 艘舰完成换层`:''}</p>}
    {detached&&distanceState?.individual_distances_m?.[shipId]!==undefined&&<p>本舰距敌旗舰 {(distanceState.individual_distances_m[shipId]/1000).toFixed(2)} 公里</p>}
    {shipId!==directId&&<>
      ...
      {distanceState&&<><button disabled={disabled} onClick={()=>onCommand({kind:detached?'cancel_detach':'detach',arguments:{}})}>{detached?'取消单舰撤离':'单舰脱队撤离'}</button>
        <p>本舰须远离敌旗舰超过有效阈值才离场；成功后转为战略 NPC，不能直接再次部署。</p></>}
    </>}
    ...
    {retreat&&<p>舰队按最慢可机动成员协调航速；推进失能舰留待抢修，整队离场时仍未恢复则留下残骸。</p>}
  </section>;
}
