...
export interface MissilePerformance {seeker:string;boost_s:number;powered_s:number;coast_s:number[];speed_cap_mps:number;max_g:number;durability:number;datalink:boolean;lost_behavior:string;warhead_scale:number;range_m:number[];seeker_range_m:number[]}
...
export type MissileOrder={module_id:string;kind:string;model_id?:string;warhead_id?:string;quantity?:number;enabled?:boolean;target_id?:string|number;point_m?:number[];layer?:string;projectile_id?:number};
...
export interface MissileLauncherView {module_id:string;target_id:string|number|null;active_target_id?:string|number|null;interceptor?:boolean;integrated_fire_control?:boolean;point_m:number[]|null;attack_layer:string;angle_rad:number;aim_point_m:number[]|null;fire_requested:boolean;shots:number;status:string;supported:boolean;maximum_range_m:number;fire_arc?:LauncherFireArc}
