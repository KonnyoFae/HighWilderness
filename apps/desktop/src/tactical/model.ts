...
export interface TacticalRequest {
  ...
  method: "tactical.create" | "tactical.inspect" | "tactical.close";
...
export function acceptSnapshot(current: TacticalView | null, next: TacticalSnapshot, instance: string): TacticalView {
  ...
  const geometry = next.static ?? (current?.snapshot.static_sha256 === next.static_sha256 ? current.geometry : null);
