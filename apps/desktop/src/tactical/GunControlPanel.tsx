...
export function GunControlPanel({ view, shipId, weaponId, groupId, disabled, onWeapon, onGroup, onCommand }: {
  ...
  return <fieldset aria-label="普通火炮操作" className="gun-control-panel">
    <legend>旗舰武器组</legend>
    <p>火炮默认自动选敌。选择武器组后，点击战场上的敌舰或模块，即可为整组指定目标。</p>
    ...
    {!first ? <p>请选择武器组。</p> : <>
      ...
      <p className="gun-order-summary" role="status">{mode === 'manual' ? '手动瞄准' : policy === 'automatic' ? '自动选敌' : policy === 'assigned' ? '指定目标' : policy === 'hold' ? '停止开火' : '组内指令不同'}
      ...
      {mode === 'manual' && <label>瞄准甲板<select aria-label="瞄准甲板" value={commonGunValue(guns,g=>g.deck_level ?? 0) ?? 'mixed'}
        onChange={e => onCommand({kind:'deck',arguments:{level:Number(e.target.value)}})}>
